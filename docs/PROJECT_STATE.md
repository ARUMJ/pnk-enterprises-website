...
preparation, real product photography, owner portrait), Phase 3C (visual
enhancement and art direction), Phases 3D–3F (successive founder-portrait
attempts, **all superseded**), **Phase 3G (the client's real founder
photograph — live and client-approved)** and **Phase 4 (branded Open Graph
share card and this documentation truth-up — the most recent phase)**.
Details in §6, §18 and **§18C**.
...
- **Do not regenerate, replace or retouch the owner's face.** The live portrait
  is the client's **real, approved photograph** — see **§18C**, which is the
  authoritative record. §18.3, §18A and §18B describe superseded attempts and
...
phase (§18.6). The remaining outstanding item is genuine client photography for
the coolers and household-items ranges. The Open Graph image was delivered in
Phase 4 (§20).
...
**Phase 4 — Open Graph share card + documentation truth-up.** Complete. A
branded 1200x630 card is generated by `src/app/opengraph-image.tsx`, replacing
the grey `/images/placeholder.svg` that social platforms previously received.
Full detail in **§20**.

**Phase 3G — The live founder portrait.** Complete, shipped and
**client-approved**. `/about` serves the client's **real, unretouched
photograph**. It is **not AI-generated**. Full detail in **§18C**, which is the
authoritative portrait record.

Phases 3D, 3E and 3F were earlier attempts at that portrait and are **fully
superseded** — see §18A and §18B for the historical record. Their instructions
must not be followed.

No dependencies have been added in any of these phases. No component was
redesigned.
...
| 3D    | Premium CEO / founder portrait                       | Founder portrait rebuilt as an AI wardrobe/background composite over his real face. **Superseded by 3G.**                                |
| 3E–3F | Founder-portrait refinements                         | Further generative refinements of the composite. **Superseded by 3G.**                                                                   |
| 3G    | The client's real founder photograph                 | Generative pipeline abandoned. `/about` now serves the client's own photograph, deterministically encoded. **Client-approved and live.** |
| 4     | Open Graph share card & documentation truth-up       | Branded 1200x630 card generated by `next/og`; placeholder OG asset retired; this document reconciled with reality. **This phase.**       |
...
- **`src/lib/siteMeta.ts`** provides:
  ...
  - `ogImage` — the descriptor pointing at the generated share card, used by
    `pageMetadata()` because a nested `openGraph` object replaces the parent's.
    See **§20.3**.
- **Open Graph image** — generated at build time by
  `src/app/opengraph-image.tsx` (1200x630 PNG, `next/og`). Full detail in
  **§20**.
...
1. **Product photography is real but partial.** Six authentic client product
   photographs are live (§18.4). Two category images — coolers and household
   items — remain **illustrative** and are labelled as such on the page; they
   are the only imagery awaiting real client photography (§18.1).
2. **Branding is the client's own.** The client's logo is prepared into brand
   marks and app icons (Phase 3B). The palette and type remain the approved
   design-system choices described in §10.
...
10. **The founder portrait is complete.** It is the client's real, approved
    photograph and is **not** a limitation or an open item. See §18C.
11. **Browser-based visual verification was not possible from the build sandbox.**
...
1. **Cooler and household-item photography** — the last two ranges still shown
   with illustrative artwork. (The other six ranges now have real photography,
   supplied in Phase 3B.)
2. ~~**Brand assets** — logo files.~~ **Supplied** in Phase 3B and live.
...
11. ~~**An owner photograph** for the `OwnerPortrait` slot on `/about`.~~
    **Supplied, approved and shipped** in Phase 3G — see §18C.
...
### 18.3 How the founder photograph was edited (Phase 3C — HISTORICAL, superseded by §18C)

> **HISTORICAL — do not follow as instructions.** The portrait described in
> this subsection (pale blue traditional outfit, warm neutral studio falloff)
> was the Phase 3C asset. It was replaced in Phase 3D (§18A), then in Phase 3E
> (§18B), and finally in **Phase 3G (§18C)** by the client's own real
> photograph, which is what the site serves today. This subsection is kept as
> the provenance record of an intermediate step. The no-generative-editing
> principle it states still applies; its file paths and steps do not.
...
- ~~**A real Open Graph image.**~~ **Delivered in Phase 4** — a branded
  1200x630 card generated by `src/app/opengraph-image.tsx` (§20).
...
## 18A. Phase 3D — The First Approved Founder Portrait (HISTORICAL — SUPERSEDED, see §18C)
...
### 18B.11 Shipping record (Phase 3E — HISTORICAL, superseded by §18C.5)
...
- **The founder portrait is the client's real photograph — never generate a
  face.** The live portrait is the client's own approved photograph, mastered
  from `public/images/source/client_photo.png`. Never generate, swap, reshape,
  beauty-filter, de-age or AI-edit the owner's face, and never place him on a
  generated background. **Do not regenerate or replace the portrait without
  explicit authorisation from the project owner** (§18C.6). The RMSE
  verification described in §18A/§18B belonged to the superseded generative
  pipeline and no longer applies.
...
## 20. Phase 4 — The Open Graph Share Card

Until this phase every social share of the site advertised
`/images/placeholder.svg`: a 387-byte, 240x160 grey SVG. Most scrapers refuse
to render SVG at all, so links posted to WhatsApp, Facebook, LinkedIn and X
appeared with no imagery — the single most visible SEO defect left in the
project.

### 20.1 What was built

`src/app/opengraph-image.tsx` — the App Router **`opengraph-image` file
convention**. Next renders it once at build time into a static **1200x630 PNG**
served from `/opengraph-image`, and injects `og:image`, `og:image:width`,
`og:image:height`, `og:image:type` and `og:image:alt` automatically.

The card is a two-panel composition in the existing design system:

| Element    | Source                                                                     |
| ---------- | -------------------------------------------------------------------------- |
| Background | `--color-ink-950` `#0e0c0a`                                                |
| Brand mark | `public/images/brand/pnk-mark-bone.png` — the client's own logo            |
| Wordmark   | `PNK ENTERPRISES`, letter-spaced, bone                                     |
| Location   | Built from `localBusiness.json` → `LAGOS ISLAND, LAGOS · NIGERIA`          |
| Headline   | "Household, kitchen & home appliance products"                             |
| Rule/pills | `--color-brass-400` `#cc9f4a`, pill labels = the five recorded categories  |
| Photograph | `public/images/products/crown-star-aluminium-cookware-set.jpg` — authentic |
| Footer     | `PETER N KABAI ENTERPRISES`, the recorded legal name                       |

### 20.2 Content-integrity rules observed

- **Every string derives from `src/data/localBusiness.json`.** No claim,
  statistic, award, certification, customer count, price or founding date was
  introduced.
- The photograph is one of the **six authentic client product photographs**
  already shipped on the site. It is framed to the cookware itself so that the
  packaging's third-party text — the `10PCS` badge, the "ONE YEAR WARRANTY"
  style marks and the manufacturer's feature list — is **outside the crop**. No
  other brand's claim is presented as a PNK claim.
- **The founder portrait is deliberately not used** on the share card.
- **No dependency was added.** `next/og` ships with Next.js.

### 20.3 Two implementation details worth knowing

**Fonts.** Satori (inside `next/og`) cannot resolve the site's CSS system-font
stacks (`ui-sans-serif`, `ui-serif`, …) because there is no font _file_ behind
them. Rather than add a webfont dependency, the card uses the font bundled with
`next/og` and builds hierarchy from **size, letter-spacing and colour** instead
of font weight. If you ever need the site's exact display face here, you must
commit a real `.ttf` and pass it in `fonts:` — do not assume a CSS stack works.

**Metadata inheritance — the subtle one.** The root segment picks the generated
card up automatically, so `defaultMetadata` in `src/lib/siteMeta.ts`
deliberately declares **no** `images` array. But a nested segment that supplies
its own `openGraph` object **replaces** the parent's, image included — so when
`pageMetadata()` set `openGraph` without images, `/about`, `/contact`,
`/products` and all five category routes silently emitted **no `og:image` at
all**. `pageMetadata()` therefore points at the same generated asset explicitly
via the exported `ogImage` descriptor. Keep its width/height/type in sync with
`size`/`contentType` in `opengraph-image.tsx`.

`public/images/placeholder.svg` was **kept**, not deleted.

### 20.4 Verification performed

- `/opengraph-image` returns **HTTP 200**, `content-type: image/png`,
  **1200x630** — confirmed with `identify` against the running production
  build, and inspected visually.
- `og:image`, `og:image:width=1200`, `og:image:height=630` and
  `og:type=website` present on `/`, `/about`, `/contact`, `/products` and the
  category routes.
- All **14** endpoints return 200; canonical URLs unchanged; JSON-LD still
  parses (`@graph` → `LocalBusiness` + `WebSite`, plus `BreadcrumbList`);
  102 `next/image` references on `/products` intact.
- Founder portrait files **unmodified** — MD5s unchanged
  (`client_photo.png` `c1f523039966d5ad89e8129f820ae27b`,
  `owner-portrait.webp` `bd167e7c9a306945920deafd714728fe`).
- All four gates green: lint, typecheck, format check, production build.

**Local builds emit `http://localhost:3000` OG and canonical URLs.** That is
`resolveSiteUrl()`'s documented fallback, not a bug; Vercel builds emit the
real host.
...
**VERCEL PREVIEW URL:** https://pnk-enterprises-website-6u2yiqo3e-gospelboys.vercel.app (Phase 3G — the Preview the client approved). Note: `*.vercel.app` is TLS-blocked from the build sandbox — read deployment state via `gh api repos/{owner}/{repo}/deployments/{id}/statuses`, not by fetching the URL.
**DEPLOYED WHERE:** Preview only. Nothing is in production, because `main` has never been advanced and there is no production domain yet.

**THE FOUNDER PORTRAIT — the one thing not to get wrong:**
The live portrait is the client's **real photograph**. It is **not
AI-generated**: there is no generated face, wardrobe, lighting or background
anywhere in it. It has been **reviewed and approved by the client** and is
**already shipped**. The only processing applied was deterministic
re-encoding — no generative step of any kind.

**Do not regenerate, replace, retouch or "improve" it without explicit
authorisation from the project owner.** Any earlier instruction describing an
AI-composited suit or background, or pupil-normalised RMSE re-verification,
